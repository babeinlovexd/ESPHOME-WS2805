...
void WS2805LightOutput::write_state(light::LightState *state) {
  ...
  if (this->is_effect_active()) {
    ...
    // writes per-LED RGB through the color views each frame. We only inject the
    // global white channels without touching the RGB bytes, allowing the user
    // to mix CCT sliders into the active effect.
    int n = this->size();
    for (int i = 0; i < n; i++) {
    ...
    // No effect active: write ALL 5 channels from remote_values. buf_ is the
    // single source of truth for the solid-color path.
